...
import EditTenantSkeleton from "./loading"; // Pastikan kamu punya file loading.tsx
...
function EditTenantPage() {
  ...
  const { isSuperAdmin } = useAuth(); // Ambil info role
    ...
    try {
      ...
      setTimeout(() => {
        if (isSuperAdmin) {
          ...
          // Jika owner edit profil sendiri, mungkin refresh halaman saja atau ke dashboard
            ...
            <Form {...form}>
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  {/* Status hanya bisa diubah oleh Super Admin */}
                  {isSuperAdmin && (
                    <FormField
                      form={form}
                      name="is_active"
                      type="select"
                      label="Status Aktifasi (Super Admin Only)"
                      placeholder="Pilih status"
                      disabled={isLoading}
                      selectOptions={[
                        { value: "true", label: "Aktif (Active)" },
                        { value: "false", label: "Non-Aktif (Suspended)" },
                      ]}
                    />
                  )}
                </div>
