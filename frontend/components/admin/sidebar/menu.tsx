...
import { LogOut, User, Settings, ChevronRight, Ellipsis } from "lucide-react";
    ...
    <div className="h-full flex flex-col">
      ...
      <ScrollArea className="flex-1 [&>div>div[style]]:!block">
        <nav className="w-full">
          <ul className="flex flex-col items-start space-y-1 px-3 pb-4">
            {menuList.map(({ groupLabel, menus }, index) => (
              <li
                className={cn("w-full", groupLabel ? "pt-4" : "")}
                key={index}
              >
      ...
      {/* Sticky User Dropdown - Always visible at bottom */}
      <div className="border-t bg-white dark:bg-zinc-950 px-3 py-3">
        {user ? (
          <DropdownMenu>
            <TooltipProvider disableHoverableContent>
              <Tooltip delayDuration={100}>
                <TooltipTrigger asChild>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      className={cn(
                        "w-full justify-start rounded-3xl h-10 group",
