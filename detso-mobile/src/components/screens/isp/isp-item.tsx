...
<Card className="mb-4 overflow-hidden border-border/40">
      <TouchableOpacity 
        ...
        onPress={() => router.push(`/isp/${item.id}`)}
      >
        <Avatar 
        ...
        <View className="flex-1 ml-4">
          <View className="flex-row justify-between items-start mb-1">
            <Text weight="bold" className="text-base text-foreground flex-1 mr-2" numberOfLines={1}>
              {item.name}
            </Text>
            <Badge 
          ...
          <Text className="text-xs text-muted-foreground mb-3" numberOfLines={1}>
          ...
          <View className="flex-row gap-x-4">
            <View className="flex-row items-center">
              ...
              <Text weight="semibold" className="text-[11px] text-primary">
                {item.stats.total_customers.toLocaleString()} {t("isp.customers")}
              </Text>
            </View>
            <View className="flex-row items-center">
              <View className="w-2 h-2 rounded-full bg-orange-500 mr-1.5" />
              <Text weight="semibold" className="text-[11px] text-orange-500">
                {item.stats.active_services.toLocaleString()} {t("isp.services")}
              </Text>
            </View>
          </View>
        </View>

        <Ionicons name="chevron-forward" size={18} color="hsl(var(--muted-foreground))" className="ml-2" />
      </TouchableOpacity>
    </Card>
  );
}